...
const SITE_NAME = "prosel-safety-hub";
...
const requestSchema = z
  .object({
    ...
    template_name: z.string().trim().min(1).max(100).optional(),
  ...
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        const keyBytes = new TextEncoder().encode(`transactional-email:${user.id}`);
        const keyHash = Array.from(new Uint8Array(await crypto.subtle.digest("SHA-256", keyBytes)))
          .map((byte) => byte.toString(16).padStart(2, "0"))
          .join("");
        const { data: withinLimit } = await supabase.rpc("consume_endpoint_rate_limit", {
          _key_hash: keyHash,
          _route: "/lovable/email/transactional/send",
          _limit: 30,
          _window_seconds: 3600,
        });
        if (withinLimit !== true) {
          return Response.json({ error: "Too many email requests" }, { status: 429 });
        }
        ...
        let rawTemplateData: Record<string, unknown> = {};
        try {
        ...
        const parsedTemplateData = template.dataSchema.safeParse(rawTemplateData);
        if (!parsedTemplateData.success) {
          return Response.json({ error: "Invalid template data" }, { status: 400 });
        }
        const templateData = parsedTemplateData.data;
