ALTER TABLE public.company_report_settings
  DROP CONSTRAINT IF EXISTS company_report_settings_letterhead_path_check;

ALTER TABLE public.company_report_settings
  ADD CONSTRAINT company_report_settings_letterhead_path_check
  CHECK (
    letterhead_path IS NULL
    OR letterhead_path ~ '^letterhead/[0-9a-f-]{36}\.(jpg|jpeg|png|webp|pdf)$'
  );
...
BEGIN
  ...
  FROM public.company_report_settings
  ...
  LOOP
    BEGIN
      ...
      FOREACH _recipient IN ARRAY _subscription.recipients LOOP
        ...
        WHERE email = _normalized_recipient
          AND used_at IS NULL
        LIMIT 1;
        ...
        IF _unsubscribe_token IS NULL THEN
          ...
          VALUES (_unsubscribe_token, _normalized_recipient)
